...
| -------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `VITE_DEPLOY_ENV`                | No       | Deploy environment (`production` / `staging` / `preview`). `"production"` hides source maps from browsers (any other value serves them publicly); `production`/`staging` also tag Sentry releases. `"preview"` enables preview-API routing, so a branch preview talks to its matching API preview; it is hard-gated to this value and dead-code eliminated from every other build. |
...
| `VITE_STAGING_API_PREVIEW_HOST`  | No       | API preview host suffix (`<api-worker>.<subdomain>`) used to build the matching API preview URL. Only read when `VITE_DEPLOY_ENV=preview`.                                                                                                                                                                                                                                         |
| `VITE_STAGING_WEB_PREVIEW_HOST`  | No       | Web preview host suffix (`<web-worker>.<subdomain>`) matched to detect a preview host and extract its alias. Only read when `VITE_DEPLOY_ENV=preview`.                                                                                                                                                                                                                             |
