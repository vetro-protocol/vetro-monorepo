...
describe("getMintFee", function () {
  ...
  it("should call readContract if all parameters are valid", async function () {
    const mintFee = BigInt(100);

    vi.mocked(readContract).mockResolvedValueOnce(mintFee);

    const result = await getMintFee(client, validParameters);
