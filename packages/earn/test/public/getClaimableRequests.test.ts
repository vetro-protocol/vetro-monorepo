...
import { getClaimableRequests } from "../../src/actions/public/getClaimableRequests.ts";
...
describe("getClaimableRequests", function () {
  it("should throw an error if client is not defined", async function () {
    await expect(
      // @ts-expect-error - Testing invalid input
      getClaimableRequests(undefined, validParameters),
    ).rejects.toThrow("Client is not defined");
  });

  it("should throw an error if parameters are not provided", async function () {
    // @ts-expect-error - Testing invalid input
    await expect(getClaimableRequests(client, undefined)).rejects.toThrow(
  ...
  it("should throw an error if the address is not valid", async function () {
    ...
    // @ts-expect-error - Testing invalid input
    await expect(getClaimableRequests(client, parameters)).rejects.toThrow(
  ...
  it("should call readContract if all parameters are valid", async function () {
    ...
    vi.mocked(readContract).mockResolvedValueOnce(claimableRequests);

    const result = await getClaimableRequests(client, validParameters);
    ...
    expect(result).toEqual(claimableRequests);
  });
});
