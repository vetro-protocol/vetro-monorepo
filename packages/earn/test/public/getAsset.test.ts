...
import { getAsset } from "../../src/actions/public/getAsset";
...
describe("getAsset", function () {
  it("should throw an error if client is not defined", async function () {
    await expect(
      ...
      getAsset(undefined, validParameters),
    ).rejects.toThrow("Client is not defined");
  });

  it("should throw an error if parameters are not provided", async function () {
    // @ts-expect-error - Testing invalid input
    await expect(getAsset(client, undefined)).rejects.toThrow(
  ...
  it("should throw an error if the address is not valid", async function () {
    ...
    // @ts-expect-error - Testing invalid input
    await expect(getAsset(client, parameters)).rejects.toThrow(
  ...
  it("should call readContract if all parameters are valid", async function () {
    ...
    const result = await getAsset(client, validParameters);
