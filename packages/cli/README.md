...
| `vetro-cli swap cooldown --gateway <addr>`                | `getWithdrawalDelayEnabled`, `getWithdrawalDelay` | Redeem Queue cooldown of the gateway, in seconds. `0` when the delay is disabled — a redeem is then one-step, with no queue.                                            |
